class Game {
  constructor() {
    ...
    this.resetButtom = createButton("killer Queen Bites the Dust");
  ...
  start() {
    ...
    var obstacles1Positions = [ 
      ...
      { x: width / 2 + 220, y: height - 1800, image: obstacle1Image }, 
  ...
  handleElements() { 
    ...
    this.resetTitle.html("reiniciar o mundo");
    this.resetTitle.class("resetText");
    this.resetTitle.position(width/2+200,40);

    this.resetButtom.class("resetText");
  ...
  handlePlayerControls() {
    ...
    if (keyIsDown(LEFT_ARROW) && player.positionX > width / 3 - 50) { 
      this.leftKeyActive = true//novo 
      player.positionX -= 5; 
      player.update(); 
    }
    if (keyIsDown(RIGHT_ARROW) && player.positionX < width / 2 + 300) {
      this.leftKeyActive = false//novo 
      player.positionX += 5; 
      player.update(); 
    }  
    }
  ...
  play() {
    ...
    var index = 0 
     for (var plr in allPlayers){ 
      ...
      if (currentLife <=0) {
        cars[index-1].changeAnimation("blast", blastImage);
    ...
    //para identificar o jogador ativo na janela 
    stroke(10); 
    fill("red"); 
    ellipse(x,y,60,60);
 ...
 gameOver() {
  swal ({
    title: `FIM DE JOGO`,
    text: "gasolina tá cara amigo! AGORA PAGUE MIL REAIS!", 
